...
enum class BusVoltageRange { RNG_16V, RNG_32V, RNG_60V };
/* Maximum measurable voltage on the shunt resistor */
enum class ShuntVoltageRange { RNG_40mv, RNG_80mv, RNG_160mv, RNG_320mv };
/* ADC possible configuration */
enum class AdcConf {
  CFG_12bit,
  CFG_13bit,
  CFG_14bit,
...
/* the enables measures (only shunt voltage, only bus voltage or both)*/
enum class MeasureEnabled { SHUNT, VOLTAGE, BOTH_SHUNT_AND_VOLTAGE };
/* the kind of measure performed (just on demand or continuosly)*/
enum class MeasureTrigger { ON_DEMAND, CONTINUOSLY };
/* just a replacement for ShuntVoltageRange
 * Scale_0 is equivalent to ShuntVoltageRange::RNG_40ms - the smallest range
 * Scale_1 is equivalent to ShuntVoltageRange::RNG_80ms
 * Scale_0 is equivalent to ShuntVoltageRange::RNG_160ms
 * Scale_0 is equivalent to ShuntVoltageRange::RNG_320ms - the widest range
 * */
enum class CurrentScale { Scale_0, Scale_1, Scale_2, Scale_3 };
...
public:
  ...
  void setShuntAdcCfg(AdcConf cfg) { shunt_adc_cfg = cfg; }
  ...
  float getShuntResistor() { return r_shunt_value_ohm; }
  void setShuntResistor(float o) { r_shunt_value_ohm = o; }
