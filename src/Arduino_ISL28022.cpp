...
#ifdef DEBUG_CFG
      Serial.println("Only Shunt measure on demand config value = " +
                     String(rv));
#endif
...
#ifdef DEBUG_CFG
      Serial.println("Only Shunt measure continuosly config value = " +
                     String(rv));
#endif
    }
...
#ifdef DEBUG_CFG
      Serial.println("Only BUS measure continuosly config value = " +
                     String(rv));
#endif
    }
...
#ifdef DEBUG_CFG
      Serial.println("BUS & SHUNT measure on demand config value = " +
                     String(rv));
#endif
...
#ifdef DEBUG_CFG
      Serial.println("BUS & SHUNT measure continuosly config value = " +
                     String(rv));
#endif
    }
  } else {
...
ISL28022Class::ISL28022Class(float _shunt_res_ohm)
    : _wire(Wire), slave_address(DEFAULT_SLAVE_ADDRESS), cfg{_shunt_res_ohm} {}
...
ISL28022Class::ISL28022Class(float _shunt_res_ohm, TwoWire &w)
    : _wire(w), slave_address(DEFAULT_SLAVE_ADDRESS), cfg{_shunt_res_ohm} {}

/* _______________________________________________________________CONSTRUCTOR */
ISL28022Class::ISL28022Class(float _shunt_res_ohm, uint8_t a)
    : _wire(Wire), slave_address(a), cfg{_shunt_res_ohm} {}

/* _______________________________________________________________CONSTRUCTOR */
ISL28022Class::ISL28022Class(float _shunt_res_ohm, TwoWire &w, uint8_t a)
    : _wire(w), slave_address(a), cfg{_shunt_res_ohm} {}
...
bool ISL28022Class::begin(bool reset) {
  ...
  if (reset) {
    // if reset is requested a reset of the device is performed
